...
- 24 built-in commands with aliases (`new-session`, `split-window`, `select-pane`, etc.)
...
src/
  ...
  cmd/
    cmd.zig                Command registry + 24 commands
...
zmux reads `~/.zmux.conf` and `/etc/zmux.conf` on startup. The configuration syntax is compatible with tmux:
